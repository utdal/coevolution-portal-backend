...
class GeneticsTools:
    ...
    def ntToAA(self, nt_sequence):
        codontoAA = self.buildCodonToAminoAcid()
        numeric_nt = self.parseDNA(nt_sequence)
        map_codon_to_aa = np.vectorize(lambda i: codontoAA[i])
        numeric_aa = map_codon_to_aa(numeric_nt)
        numToAA ={}
        for aa, num in self.aa_code.items():
            numToAA[num]=aa
        map_num_to_aa = np.vectorize(lambda i: numToAA[i])
        aa_sequence = map_num_to_aa(numeric_aa)
